...
export default function Layout({ children, beliefs, desires, intentions, selectedDependencies }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'dark bg-slate-900 text-white' : 'bg-white'}`}>
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <nav className="hidden sm:flex items-center space-x-4">
              <Link href="/" className={`px-3 py-2 rounded-md text-sm font-medium ${location === '/' 
                ? 'bg-blue-100 text-blue-700' 
                : isDarkMode ? 'text-slate-300 hover:text-white' : 'text-slate-700 hover:text-slate-900'}`}>
                Home
              </Link>
              
              <Link href="/bdi" className={`px-3 py-2 rounded-md text-sm font-medium flex items-center ${location === '/bdi' 
                ? 'bg-blue-100 text-blue-700' 
                : isDarkMode ? 'text-slate-300 hover:text-white' : 'text-slate-700 hover:text-slate-900'}`}>
                BDI State
      ...
      <main className={`container mx-auto px-4 py-8 flex-1 ${isDarkMode ? 'text-white' : ''}`}>
        {children}
      </main>

      <footer className={`border-t py-6 ${isDarkMode ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="mt-4 md:mt-0 flex items-center space-x-4">
              ...
              <div className="flex items-center">
                <Link href="/bdi" className={`flex items-center text-sm font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>
