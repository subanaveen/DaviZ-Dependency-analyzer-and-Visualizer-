          ...
          <div className="network-graph h-[700px] w-full bg-slate-50 border border-slate-200 rounded-lg">
            {showGraph ? (
              ...
              >
                ...
                {/* The graph would be dynamically generated here */}
                ...
                {Object.keys(selectedDependencies).length > 0 && Object.keys(dependencies).length > 0 && (
                  <>
                    {/* First level nodes example */}
                    {Object.keys(dependencies).slice(0, 1).map((feature) => 
                      selectedDependencies[feature]?.map((dep, index) => (
                        <g key={index}>
                          <g 
                            className={`node ${expandedNodes.has(dep) ? "expanded" : ""}`} 
                          ...
                          >
                            <rect 
                              ...
                              rx="5" 
                              ry="5" 
                              className={expandedNodes.has(dep) ? "fill-green-200 stroke-green-600" : "fill-blue-200 stroke-blue-600"}
                              strokeWidth="2"
                            ></rect>
                            <text x="90" y="25" textAnchor="middle" className="text-sm font-medium fill-slate-800">
                              {dep}
